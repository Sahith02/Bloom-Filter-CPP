...
int main(int argc, char** argv){
	...
	cout << endl << "===== INT TEST CASES STARTING =====" << endl;
	auto t1 = high_resolution_clock::now();
	// Inner code for timing starts

	string TEST_FILE = "./tests/int/int_test_" + (string)argv[1] + ".txt";
	string CHECK_FILE = "./tests/check_int_exists.txt";
	...
	if(check_file.is_open()){
		...
		while (getline(check_file, line)){
			...
			cout << "Is Present? - " << line << " : " << bf.check(stoi(line)) << endl;
		}
		check_file.close();
	}
}